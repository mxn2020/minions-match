...
/**
 * @minions-match/cli — CLI for Minions match
 *
...
import { customTypes } from '@minions-match/sdk';
...
program.name('match').description('Match scoring, shortlists, and hard gate evaluation between jobs and profile').version('0.1.0');

program.command('info').description('Show project info').action(() => {
    console.log(chalk.bold('Minions match'));
    console.log(chalk.dim('Match scoring, shortlists, and hard gate evaluation between jobs and profile'));
    console.log('');
    console.log(`  SDK:    ${chalk.cyan('@minions-match/sdk')}`);
    console.log(`  CLI:    ${chalk.cyan('@minions-match/cli')}`);
    console.log(`  Python: ${chalk.cyan('minions_match')}`);
